...
def readin_base_classes(desired_class=None):
    clss = get_nexpy_classes('base_classes', desired_class)
    return (clss[desired_class])


def readin_application_classes(desired_class=None):
    clss = get_nexpy_classes('applications', desired_class)
    return (clss[desired_class])


def readin_contributed_definition_classes(desired_class=None):
    clss = get_nexpy_classes('contributed_definitions', desired_class)
    return (clss)


def get_nexpy_classes(class_dir, desired_class=None):
    ...
    for nxdl_file in nxdl_files:
        ...
        for child in xml_root:
            ...
            if child.tag.endswith('field'):
                ...
                class_fields[name] = {'type': dtype, 'units': units, 'nxstxm_nxdl': nxstxm_nxdl,
...
def is_entry_nx_type(entry, typ='NXstxm'):
    ...
    if (isinstance(entry['definition'][()], str)):
        if (entry['definition'][()] == typ):
            ret = True
    elif (isinstance(entry['definition'][()], bytes)):
        if (entry['definition'][()].decode('utf-8') == typ):
            ret = True
...
def tabbed_print(num_tabs, msg, lvl=0):
    '''
    lvls:   0 is info (normal)
            1 is warning
            2 is error
            3 is decorative
    ascii codes taken from http://ozzmaker.com/add-colour-to-text-in-python/

    The escape codes are entered right into the print statement.
    1   print("\033[1;32;40m Bright Green  \n")
    ...
    for i in range(num_tabs):
        s += '\t'
    if (lvl is 0):
        s += '\033[1;30;39m ' + msg + '\033[1;30;39m '
    elif (lvl is 1):
        s += '\033[1;33;40m ' + msg + '\033[1;30;39m '
    elif (lvl is 2):
        s += '\033[1;31;40m ' + msg + '\033[1;30;39m '
    elif (lvl is 3):
        s += '\033[1;37;44m ' + msg + '\033[1;30;39m '
...
def print_check_warning(entry_str, msg):
    num_tabs = 1
    tabbed_print(num_tabs + 1, '\033[1;32;40mWarning: in entry %s: %s \033[1;30;39m' % (entry_str, msg))


def print_check_error(entry_str, msg):
    num_tabs = 1
    tabbed_print(num_tabs + 1, '\033[1;31;40m ERROR in entry %s: %s \033[1;30;39m' % (entry_str, msg))
...
def check_nxcollection_group(nf):
    ...
    num_tabs = 1
    res = True
    tabbed_print(num_tabs, '### checking for NXcollection group')
    col_nam, col_grp = get_nx_group_from_entry(nf, 'NXcollection')
    if (col_grp is None):
        tabbed_print(num_tabs, 'an NXcollection group does not exist in the entry')
        return

    tabbed_print(num_tabs, 'found NXcollection group called <%s>' % col_nam)

    if (not check_grp_and_nxclass(nf, col_nam, 'NXcollection')):
        tabbed_print(num_tabs, 'NXstxm file has no or non-standard NXcollection group named [collection]')
        res = False
        return (res)
    else:
        tabbed_print(num_tabs + 1, '> it exists and type=NXcollection')

    # check keys of collection group
    tabbed_print(num_tabs + 2, '### checking <collection> fields:')
    if (not check_fields_of_nxclass(nf['collection'], nexus_grp_dct={'scan_request': 'NXscanDefinition'})):
        tabbed_print(num_tabs + 3, 'NXcollection group does not contain correct set of fields')
        res = False

    return (res)


def get_control_axes_names(attrs, remove_indices=False):
    lst = []
    for f in attrs.keys():
        if (f.find('axes') > -1):
            if (remove_indices):
...
def check_nxcontrol_group(nf, nxstxm_nxdl=None):
    ...
    if (cntrl_grp is None):
        tabbed_print(num_tabs, 'an NXmonitor group does not exist in the entry')
        return

    tabbed_print(num_tabs, '### checking OPTIONAL NXmonitor group called <%s>:' % cntrl_nam)
    if (not check_grp_and_nxclass(nf, cntrl_nam, 'NXmonitor')):
        tabbed_print(num_tabs, 'NXstxm file has no or non-standard NXmonitor group named [%s]' % cntrl_nam)
        res = False
        return (res)
    else:
        tabbed_print(num_tabs + 1, '> it exists and type=NXmonitor')
    # check attrs of NXcollection
    fld_lst = get_control_axes_names(nf[cntrl_nam].attrs) + ['signal']
    tabbed_print(num_tabs + 2, '### checking <%s> attributes:' % cntrl_nam)
    # attr_lst contains the minnimum attributes required
    if (not check_attrs_of_nxclass(nf[cntrl_nam], attr_lst=fld_lst)):
        tabbed_print(num_tabs + 3, 'NXmonitor group does not contain correct set of attrs')
        res = False
    # check keys of collection group
    # fld_dct = get_NXstxm_NXentry_fieldnames(cntrl_grp)
    fld_lst = get_control_axes_names(nf[cntrl_nam].attrs, remove_indices=True) + ['data', 'energy']
    tabbed_print(num_tabs + 2, '### checking <%s> fields:' % cntrl_nam)
    # fld_dct = get_NXstxm_NXentry_fieldnames(cntrl_grp)
...
def check_nxdata_group(nf, cntr_lst=[], nxstxm_nxdl=None):
    ...
    for cntr in cntr_lst:
        tabbed_print(num_tabs, '### checking for NXdata group called <%s>:' % cntr)
        if (not check_grp_and_nxclass(nf, cntr, 'NXdata')):
            tabbed_print(num_tabs, 'NXstxm file has no or non-standard NXdata group named [%s]' % cntr)
            res = False
            break
        else:
            tabbed_print(num_tabs + 1, '> it exists and type=NXdata')

        # check attrs of NXcollection
        tabbed_print(num_tabs + 2, '### checking <%s> attributes:' % cntr)
        fld_lst = get_control_axes_names(nf[cntr].attrs) + ['axes', 'signal']
        ...
        if (not check_attrs_of_nxclass(nf[cntr], attr_lst=fld_lst)):
            tabbed_print(num_tabs + 3, 'NXdata group does not contain correct set of attrs')
            res = False
        # check keys of collection group

        tabbed_print(num_tabs + 2, '### checking <%s> fields:' % cntr)
        ...
        if (not check_fields_of_nxclass(nf[cntr], nexus_grp_dct=fld_dct)):
            tabbed_print(num_tabs + 3, 'NXdata group does not contain correct set of fields')
            res = False
        else:
            ...
            if (isinstance(nf_stxm_scan_type, str)):
                ...
                if (str_in_lst(nf_stxm_scan_type, nxstxm_scan_type_lst)):
                    tabbed_print(num_tabs + 3, 'stxm_scan_type = <%s> is a valid type' % nf_stxm_scan_type)
                else:
                    tabbed_print(num_tabs + 3,
                                 'Error: stxm_scan_type = <%s> is an invalid stxm_scan type' % nf_stxm_scan_type)
            else:
                tabbed_print(num_tabs + 3, 'Error: stxm_scan_type is not of type NX_CHAR')
...
def check_nxinstrument_group(nf, cntr_lst=[], nxstxm_nxdl=None):
    ...
    tabbed_print(num_tabs, '### checking for NXinstrument group called <instrument>:')
    if (not check_grp_and_nxclass(nf, 'instrument', 'NXinstrument')):
        tabbed_print(num_tabs, ' !! NXstxm file has no or non-standard NXinstrument group named [instrument]')
        res = False
        return (res)
    else:
        tabbed_print(num_tabs + 1, '> it exists and type=NXinstrument')

    # check keys of collection group
    tabbed_print(num_tabs + 2, '### checking <instrument> fields:')
    ...
    if (not check_nxsource_group(nf['instrument'], nxstxm_nxdl=nxstxm_nxdl)):
        tabbed_print(num_tabs + 2, ' !! NXsource group does not contain correct set of fields')
        res = False
...
def check_nxsample_group(nf, nxstxm_nxdl=None):
    ...
    tabbed_print(num_tabs, '### checking for NXsample group called <sample>:')
    if (not check_grp_and_nxclass(nf, 'sample', 'NXsample')):
        tabbed_print(num_tabs, 'NXstxm file has no or non-standard NXsample group named [collection]')
        res = False
        return (res)
    else:
        tabbed_print(num_tabs + 1, '> it exists and type=NXsample')
    # check keys of collection group
    tabbed_print(num_tabs + 2, '### checking <sample> fields:')
    ...
    if (not check_fields_of_nxclass(nf['sample'], nexus_grp_dct=fld_dct)):
        tabbed_print(num_tabs + 3, 'NXsample group does not contain correct set of fields')
        res = False
...
def check_nxsource_group(nf, nxstxm_nxdl=None):
    ...
    num_tabs = 2
    res = True
    tabbed_print(num_tabs, '### checking for NXsource group called <source>:')
    if (not check_grp_and_nxclass(nf, 'source', 'NXsource')):
        tabbed_print(num_tabs, 'NXstxm file has no or non-standard NXsample group named [collection]')
        res = False
        return (res)
    else:
        tabbed_print(num_tabs + 1, '> it exists and type=NXsource')
    # check keys of collection group
    tabbed_print(num_tabs + 2, '### checking <source> fields:')
    ...
    if (not check_fields_of_nxclass(nf['source'], nexus_grp_dct=fld_dct)):
        tabbed_print(num_tabs + 3, 'NXsource group does not contain correct set of fields')
        res = False
    ...
    if (hasattr(nxgrp, 'attrs')):
        for a in attr_lst:
            if a not in nxgrp.attrs.keys():
                tabbed_print(num_tabs, 'ATTR[%s] > DOES NOT EXIST' % (a), lvl=2)
                ret = False
            else:
                tabbed_print(num_tabs, 'attr[%s] > it exists' % a)

            # if(a.find('axes') > -1):
            #     #check that a data group for the axes listed exists
...
def check_for_corresponding_data_group(nxgrp, nm):
    '''
    given a list of axes attribute names, check to see if there also exists a data group for each axis name
    in this parent group (nxgrp)
    :param nxgrp:
    :param axis_nm:
    :return:
    '''
    ...
    num_tabs = 4
    if (nm in nxgrp.keys()):
        tabbed_print(num_tabs, 'attr[%s] has a corresponding datagroup in this group' % (nm))
        return (True)
    else:
        tabbed_print(num_tabs, 'ATTR[%s] > DOES NOT HAVE A CORRESPONDING DATAGROUP IN THIS GROUP' % (nm), lvl=2)
        return (False)
    ...
    for f in nexus_grp_lst:
        if f not in nf_keys:
            if (f in nexus_grp_opt_lst):
                tabbed_print(num_tabs, 'field[%s] > DOES NOT EXIST but it is optional' % (f), lvl=1)
            else:
                tabbed_print(num_tabs, 'FIELD[%s] > DOES NOT EXIST' % (f), lvl=2)
                ret = False
        else:
            fld_typ = get_field_NX_class_type(nf[f])
            if (fld_typ == nexus_grp_dct[f]):
                tabbed_print(num_tabs, 'field[%s] exists and type=%s is correct' % (f, fld_typ))
            else:
                tabbed_print(num_tabs, 'field[%s] exists but type=%s is INCORRECT it should be %s' % (
                f, str(type(nf[f][()])), nexus_grp_dct[f]), lvl=2)
...
def check_fields_of_nxclass_list(nf, nexus_grp_lst=[]):
    ...
    for f in nexus_grp_lst:
        if f not in nf_keys:
            ...
            tabbed_print(num_tabs, 'field[%s] it exists' % (f))
    ...
    for nexus_cname in nf_class_names:
        if nexus_cname not in nxgrp_class_lst:
            ...
            if nexus_cname in optional_classes:
                tabbed_print(num_tabs, 'NX_class[%s] > EXISTS but it is OPTIONAL' % (nexus_cname), lvl=1)
            else:
                tabbed_print(num_tabs, 'NX_class[%s] > DOES NOT EXIST' % (nexus_cname), lvl=2)
                ret = False
        else:
            tabbed_print(num_tabs, 'NX_class[%s] > it exists' % nexus_cname)
    return (ret)
...
def check_stxm_file(stxm_file, nxstxm_nxdl):
    ...
    if (res):
        print('File: %s contains a valid nxstxm structure\n' % stxm_file)
    else:
        print('FAIL! File: %s contains an invalid nxstxm structure\n' % stxm_file)
    nf.close()
    return (res)
...
def validate_filesin_dir(dir):
    ...
    if (os.path.isdir(dir)):
        ...
        for f in files:
            # if(not check_stxm_file(dir + '//' + f, nxstxm_nxdl))
            if (not check_stxm_file(os.path.join(dir, f), nxstxm_nxdl)):
                invalid_files.append(os.path.join(dir, f))

    else:
        print('Directory [%s] does not exist' % dir)

    if (len(invalid_files) > 0):
        tabbed_print(0, 'There were some errors, the following files contain invalid nxstxm structures', lvl=2)
    for fname in invalid_files:
        tabbed_print(0, '[%s] FAILED nxstxm validation' % fname, lvl=2)
